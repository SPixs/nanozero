...
export class FastBoard {
  ...
  // Compte les occurrences PASSÉES du repKey de la position COURANTE dans la fenêtre [limit..histSize-1]
  // (bornée par le compteur halfmove, exactement comme toEncoderState/_isThreefold). Retourne 0/1/2
  // (clampé à 2 : ≥2 = 3-fold → la position serait déjà terminale, jamais NN-évaluée). C'est
  // l'indicateur `rep` de la clé du cache d'éval (eval-cache.mjs) — RÉUTILISE la même clé/fenêtre que
  // l'encodeur (rep1 ⇔ count≥1, rep2 ⇔ count≥2 dans les plans 12/13 par timestamp).
  _currentRepCount() {
    const snaps = this.snapshots;
    const histSize = snaps.length - 1;
    const limit = Math.max(0, histSize - this.position.halfmoves);
    const cur = snaps[histSize].repKey;
    let c = 0;
    for (let i = histSize - 1; i >= limit; i--) {
      const k = snaps[i].repKey;
      if (k.lo === cur.lo && k.hi === cur.hi && ++c >= 2) break;
    }
    return c;
  }
