...
// CACHE D'ÉVAL (eval-cache.mjs) — expansion depuis des priors par-coup-légal CACHÉS (déjà masqués/
// normalisés) : SAUTE le softmax dense→légal de `expandNode`. La génération de coups étant
// DÉTERMINISTE pour une position, `priors[i]` s'aligne sur `moves[i]` (garde de longueur côté cache).
// Les enfants produits sont bit-identiques à ceux qu'`expandNode` aurait posés (mêmes P, même `move`).
function expandNodeFromPriors(node, moves, priors) {
  node.children = moves.map((m, i) => ({ move: packMove(m), P: priors[i], node: new Node() }));
}

// Extrait les priors par-coup-légal (P des enfants) pour stockage cache. Appelé APRÈS `expandNode`
// mais AVANT tout `addDirichletNoise` → on stocke les priors NN PURS (le bruit racine est ré-échantillonné
// à chaque search, jamais caché).
function priorsOf(node) {
  const priors = new Float32Array(node.children.length);
  for (let i = 0; i < node.children.length; i++) priors[i] = node.children[i].P;
  return priors;
}
...
// `cache` (EvalCache|null) : actif uniquement en CPU/WASM (cf. worker-entry) ; null = chemin d'origine
// strictement inchangé (aucune clé calculée, aucun surcoût).
async function simulate(node, gs, net, isRoot, cache) {
  if (node.terminal) {
  ...
  if (node.children === null) {
    ...
    // CACHE : lookup AVANT net.evaluate. HIT → expansion depuis priors cachés (saute éval + masquage).
    const key = cache ? gs.cacheKey() : null;
    const hit = key ? cache.get(key, moves.length) : undefined;
    let value, wdl;
    if (hit) {
      expandNodeFromPriors(node, moves, hit.priors);
      value = hit.value; wdl = hit.wdl;
    } else {
      const planes = encodePlanes(gs.toEncoderState());
      const ev = await net.evaluate(planes);
      expandNode(node, side, ev.policy, moves);
      value = ev.value; wdl = ev.wdl || valueToWDL(ev.value);
      if (key) cache.set(key, { value, wdl, priors: priorsOf(node) }); // store APRÈS masquage, AVANT bruit
    }
    if (isRoot) addDirichletNoise(node);
    node.N++; node.W += value; addWDL(node, wdl);
    return { value, wdl };
  }
  ...
  let child;
  try {
    child = await simulate(ch.node, gs, net, false, cache);
  } finally {
 ...
 */
export async function search(gs, net, numSims, { addNoise = true, onProgress = null, root: reuse = null, evalCache = null } = {}) {
  ...
  for (let i = 0; i < numSims; i++) {
    await simulate(root, gs, net, noiseOnFirstExpand, evalCache);
...
async function runWave(root, gs, net, vl, k, cache) {
  const reqs = []; // {path:[racine..leaf], leaf, terminal, value?, planes?, side?, moves?, key?, cached?}
      ...
      for (;;) {
        ...
        if (node.children === null) {
          if (gs.isGameOver()) {
            ...
            // CACHE : lookup en PHASE 1 (gs EST à la position-feuille). HIT → on diffère l'expansion à
            // la PHASE 2 sans calculer les plans ni évaluer. MISS → on encode les plans (gs ne sera plus
            // à cette position en PHASE 2, après remontée) et on garde la clé pour le store.
            const side = gs.sideToMove();
            const moves = gs.legalMoves();
            const key = cache ? gs.cacheKey() : null;
            const cached = key ? cache.get(key, moves.length) : undefined;
            if (cached) {
              reqs.push({ path, leaf: node, terminal: false, side, moves, cached });
            } else {
              reqs.push({
                path, leaf: node, terminal: false,
                planes: encodePlanes(gs.toEncoderState()), side, moves, key,
              });
            }
          }
          break;
  ...
  // CACHE : les leaves HIT (r.cached) sont expansés depuis les priors cachés sans éval ; seuls les
  // MISS uniques alimentent le batch net.evaluate, puis sont stockés (chaque clé MISS → un set).
  const uniq = [];        // MISS uniques à évaluer
  const seen = new Set();
  const resByLeaf = new Map(); // leaf -> {value, wdl}
  for (const r of reqs) {
    if (r.terminal || seen.has(r.leaf)) continue;
    seen.add(r.leaf);
    if (r.cached) {
      expandNodeFromPriors(r.leaf, r.moves, r.cached.priors);
      resByLeaf.set(r.leaf, { value: r.cached.value, wdl: r.cached.wdl });
    } else {
      uniq.push(r);
    }
  }
  ...
  for (let i = 0; i < uniq.length; i++) {
    ...
    const wdl = ev.wdl || valueToWDL(ev.value);
    resByLeaf.set(r.leaf, { value: ev.value, wdl });
    expandNode(r.leaf, r.side, ev.policy, r.moves); // chaque leaf unique expansé UNE fois
    if (r.key) cache.set(r.key, { value: ev.value, wdl, priors: priorsOf(r.leaf) }); // store APRÈS masquage
  }
 ...
 */
export async function searchBatched(gs, net, numSims, { addNoise = true, waveSize = 8, virtualLoss = 1.0, onProgress = null, root: reuse = null, evalCache = null } = {}) {
  const root = reuse ?? new Node();
  if (gs.isGameOver()) {
  ...
  if (reuse != null && root.children !== null) {
    ...
    // Pré-expansion racine = 1ʳᵉ sim. CACHE : même schéma que simulate (lookup avant éval, store
    // les priors PURS avant le bruit Dirichlet).
    const side = gs.sideToMove();
    const moves = gs.legalMoves();
    const key = evalCache ? gs.cacheKey() : null;
    const hit = key ? evalCache.get(key, moves.length) : undefined;
    let value, wdl;
    if (hit) {
      expandNodeFromPriors(root, moves, hit.priors);
      value = hit.value; wdl = hit.wdl;
    } else {
      const planes = encodePlanes(gs.toEncoderState());
      const ev = await net.evaluate(planes);
      expandNode(root, side, ev.policy, moves);
      value = ev.value; wdl = ev.wdl || valueToWDL(ev.value);
      if (key) evalCache.set(key, { value, wdl, priors: priorsOf(root) });
    }
    if (addNoise) addDirichletNoise(root);
    root.N = 1;
    root.W = value;
  ...
  while (done < numSims) {
    done += await runWave(root, gs, net, virtualLoss, Math.min(waveSize, numSims - done), evalCache);
